...
def parse_args():
    ...
    parser.add_argument("--port", help="Port for incoming requests")
    return parser.parse_args()


def initialize_app(db_uri=None):
    ...
    settings = {
        ...
        "DB": MongoClient() if db_uri is None else MongoClient(db_uri),
        "QUEUE": Queue(),
    }
...
if __name__ == "__main__":
    ...
    app = initialize_app(db_uri=args.db_uri)
    ...
    if args.https:
        ...
        logger.info("listening on port {} (HTTPS)".format(port))
    else:
        app.listen(port)
        logger.info("listening on port {} (HTTP)".format(port))
